    ...
    for mentor_id, MentorClass, subject, test_question in mentor_tests:
        try:
            ...
            if len(system_prompt) > 200:  # Reasonable prompt length
                ...
                results.record_fail(f"{mentor.name}", f"System prompt too short ({len(system_prompt)} chars)")
...
def test_base_agent():
    ...
    try:
        ...
        valid_context = {
            'student': {'grade': 10},
            'curriculum': {'curriculum_name': 'Test'}
        }
        ...
        context_with_obj = {
            ...
            'curriculum': {
                'current_objectives': [
                    {'id': 'obj-123', 'objective_text': 'Test objective'}
                ]
            }
        }
